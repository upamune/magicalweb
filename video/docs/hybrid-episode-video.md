...
後半は**発話中のホスト一人のアイコンを字幕の左側**に配置する。暗幕・グラデーション・字幕カードは使わない。michiruはピンク、upamuneは水色の**Noto Sans JP Black（900）**に白フチと文字の影を付け、最大2行のYouTube風テロップにする。字幕の話者はCaptionPage.speaker。字幕の保持中もその話者を表示する。
...
Remotionは合成映像で両話者の字幕をレンダー。Classicは以前の比較でピクセル差分ゼロ。検証環境はChromium 153、Google Fonts通信制限のためQAのみ同名ローカルTTFを使用（Noto Sans JPはGoogle Fonts公式リポジトリの可変TTFを900で描画）。本番のフォント処理は維持する。
