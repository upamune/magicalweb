...
**クリップタイトル（必須）**: トップレベルに
`"clipTitleLines": ["帝国がクローンを", "やめた理由"]` を書く。タイトルカードに
エピソード名の代わりに表示される、ショート動画としての見出し。クリップの内容を
簡潔に表すフックにする（オチそのものはネタバレになるので書かず、興味を引く切り口で）。
1行 約8文字・最大2行。省略時はエピソードタイトル表示にフォールバックするが、基本は必ず付ける。
