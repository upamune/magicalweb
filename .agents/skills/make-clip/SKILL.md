...
動画に2人のアバターが表示され発話中の側がハイライトされる。割り当ては
**F0ピッチのクラスタリング**で行う（2人の声域は十分離れている。参考実測値:
michiru ≈ 167Hz / upamune ≈ 115Hz）:

```python
# venv に librosa を入れ、ページごとの区間で median F0 を計算して近い方に割り当てる
f0 = librosa.yin(seg, fmin=60, fmax=350, sr=sr)
f0 = f0[(f0 > 65) & (f0 < 340)]   # 有声フレームのみ
```

- 参考値とのズレが不安な場合は、**名前を呼び合う箇所**（「ウパさんは〜ですか?」の
  質問→回答など）をアンカーにしてそのエピソード内で実測し直す
- 割り当て結果は必ず**会話の文脈と突き合わせて検証**する（質問側/回答側、
  関西弁の使用、笑いながらの発話はF0が上がるので注意）
