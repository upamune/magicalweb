...
### 7. サイトへのリンク掲載（動画URLをもらったら）

レンダリングした動画をユーザーがアップロードしてURL（gyazo等）をくれたら、
`src/data/clips.json` にエピソード番号キーで追記する（1エピソードに複数可）。
エピソード詳細ページに「切り抜きクリップ」セクションとしてリンクが表示される:

```json
{ "264": [{ "url": "https://...mp4", "label": "50歳でもバリベイビー" }] }
```

`label` にはクリップのオチ・見どころを短く書く。
